...
#include "jc_type_file_unique_cwlv_private.h"

#define JC_TYPE_FILE_UNIQUE_CWLV "unique_continue_with_last_value" 

struct jc_type_file_unique_var_data {
	int cur_num;
};

struct jc_type_file_unique_cwlv {
	int alloc_num;
	int cur_id;
	struct jc_type_file_comm_hash *comm_hash;
};

static struct jc_type_file_unique_cwlv global_cwlv;

static int jc_type_file_unique_cwlv_init( 
	struct jc_comm *jcc
)
{
	...
	global_cwlv.alloc_num = jtfup->alloc_num;

	return global_cwlv.comm_hash->init(global_cwlv.comm_hash, jcc);
}

static int
jc_type_file_unique_cwlv_execute(
	struct jc_comm *jcc
)
{
...
static  int
jc_type_file_unique_cwlv_comm_init(
...
static int
jc_type_file_unique_cwlv_comm_copy(
	struct jc_type_file_comm_node *fcn,
...
)
{
	...
	var_data->cur_num = 0;
	line = global_cwlv.alloc_num * global_cwlv.alloc_num, 
...
static char *
jc_type_file_unique_cwlv_comm_execute(
	char separate,
...
)
{
	...
	if (vdata->cur_num >= global_cwlv.alloc_num)
...
int
json_type_file_unique_cwlv_uninit()
{
	struct jc_type_file_manage_oper oper;
	struct jc_type_file_comm_hash_oper comm_oper;

	memset(&comm_oper, 0, sizeof(comm_oper));
	comm_oper.comm_hash_execute = jc_type_file_unique_cwlv_comm_execute;
	comm_oper.comm_hash_init    = jc_type_file_unique_cwlv_comm_init;
	comm_oper.comm_hash_copy    = jc_type_file_unique_cwlv_comm_copy;
	global_cwlv.comm_hash = 
	...
	oper.manage_init = jc_type_file_unique_cwlv_init;
	oper.manage_copy = jc_type_file_unique_cwlv_copy;
	oper.manage_execute = jc_type_file_unique_cwlv_execute;

	return jc_type_file_unique_module_add(JC_TYPE_FILE_UNIQUE_CWLV, &oper);
}

int
json_type_file_unique_cwlv_init()
{
	if (global_cwlv.comm_hash)
		return jc_type_file_comm_destroy(global_cwlv.comm_hash);
